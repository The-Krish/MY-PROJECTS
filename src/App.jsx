...
function ArtGallery({ artProjects }) {
    ...
    return (
        <Tile className="col-span-1 md:col-span-4">
            {/* --- ArtGallery Start --- */}
            <h3 className={`text-xl font-extrabold border-b-2 ${tileAccentBorder} pb-1 mb-4 flex items-center gap-2 ${mainText}`}><Palette className="w-5 h-5"/> ART GALLERY BLOCKS</h3>
    ...
    useEffect(() => {
        ...
        const loadProfile = async () => {
            try {
                ...
                // Map server document to the portfolioData shape (fallback to empty strings)
                const mapped = {
    ...
    useEffect(() => {
        const loadExperiences = async () => {
            ...
            if (res.ok) {
                ...
                // Get experiences array from body
                let experiencesList = body.d;
                if (!experiencesList) {
                    experiencesList = body;
                }
                
                // Map each experience with an ID
    ...
    useEffect(() => {
        const loadSkills = async () => {
            ...
            if (res.ok) {
                ...
                // Get skills array from body
                let skillsList = body.d;
                if (!skillsList) {
                    skillsList = body;
                }
                
                // Map each skill with an ID
    ...
    useEffect(() => {
        const loadProjects = async () => {
            ...
            if (res.ok) {
                ...
                // Get projects array from body
                let projectsList = body.d;
                if (!projectsList) {
                    projectsList = body;
                }
                
                // Map each project with an ID
    ...
    useEffect(() => {
        const loadQueries = async () => {
            ...
            if (res.ok) {
                ...
                // Get queries array from body
                let queriesList = body.d;
                if (!queriesList) {
                    queriesList = body;
                }
                
                // Map each query with an ID
    ...
    useEffect(() => {
        const loadArt = async () => {
            ...
            if (res.ok) {
                ...
                // Get art array from body
                let artList = body.d;
                if (!artList) {
                    artList = body;
                }
                
                // Map each art with an ID
                ...
                console.error('Failed to load art');
            }
        };
